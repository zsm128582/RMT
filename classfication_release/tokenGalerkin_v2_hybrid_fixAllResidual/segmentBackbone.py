...
class BasicBlock(nn.Module):
    ...
    def forward(
        self, queries: torch.Tensor, imgs: torch.Tensor, query_pe: torch.Tensor, key_pe: torch.Tensor , h , w
        ...
        img_norm = self.norm1(imgs)
        qkNorm = self.norm1(imgs + key_pe)

        attn_out = self.attn(query= qkNorm, key=qkNorm, value=img_norm,need_weights=False)[0]

        imgs = residual + attn_out
        
        residual = imgs

        imgs =  residual + self.mlp(self.norm2(imgs))
...
class TokenAttentionLayer(nn.Module):
    def __init__(self, embed_dim, out_dim, depth, num_heads, num_q,
                 ...
                 layerscale=False, layer_init_values=1e-5 , fullAttention = False):
        ...
        if fullAttention:
            ...
            BasicBlock(self.embed_dim , num_heads , mlp_dim=ffn_dim,attention_downsample_rate=1,skip_first_layer_pe=(i == 0)) 
    ...
    def __init__(self, in_chans=3, num_classes=1000,
        ...
        for i_layer in range(self.num_layers):
            layer = TokenAttentionLayer(embed_dim=embed_dims[i_layer],
                                        ...
                                        fullAttention=True if i_layer == self.num_layers-1 else False
                                        )
...
@register_model
def tokengalerkin_v2_hybrid(args):
    model = VisSegNet(
