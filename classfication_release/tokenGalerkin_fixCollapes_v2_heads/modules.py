...
class simple_attn(nn.Module):
    def __init__(self, dim_in, heads):
        ...
        # self.mixer = nn.Conv2d(in_channels=5, out_channels=1, kernel_size=1, stride=1, padding=0)
        self.kln = nn.LayerNorm(dim_in)
        self.vln = nn.LayerNorm(dim_in)
    ...
    def forward(self, x , token):
        ...
        # b n c ->  b h n c
        # q = self.image_projection(x).unsqueeze(1).expand(-1,5,-1,-1)
        q = self.image_projection(x)
        k = rearrange( self.token_projection_k(token) , ' b (h n) c -> b h n c ' , h = self.heads ).contiguous()
        v = rearrange( self.token_projection_v(token) , ' b (h n) c -> b h n c ' , h = self.heads ).contiguous()
        ...
        # k : b h n c , v : b h n c  , q : b h n c
        attn = torch.matmul(k.transpose(-2,-1)/N, v)
        # b (h)  n c  *  b h c c -> b h  n c 
        attn = torch.softmax(attn,dim=-2)
        # attn = self.mixer(attn).squeeze(1)
        attn = torch.mean(attn , 1)
        ret = torch.matmul(q, attn)
        # ret =  self.mixer(ret).squeeze(1)
