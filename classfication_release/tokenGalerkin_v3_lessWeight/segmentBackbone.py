...
from .modules import PatchEmbed , PatchMerging , DWConv2d ,RetNetRelPos2d,MemoryEfficientSwish,theta_shift,RotateModule,simple_attn,LayerNorm
...
class FeedForwardNetwork(nn.Module):
    def __init__(
        ...
        self.fc2 = nn.Linear(ffn_dim, output_dim)
...
class TwoWayAttentionBlock(nn.Module):
    def __init__(
        ...
        self.mlp = FeedForwardNetwork(embedding_dim , embedding_dim , mlp_dim, subconv=False )
        ...
        self.queries_W = nn.Linear(embedding_dim , embedding_dim)

        self.softmax = nn.Softmax(dim=-1)
        self.ln = LayerNorm((num_heads , 1 , head_dim))

    def forward(
        self, queries: torch.Tensor, imgs: torch.Tensor, query_pe: torch.Tensor , img_pe: torch.Tensor , h , w
        ...
        imgs = self.norm1(imgs + img_pe)
        queries = self.norm1(queries+query_pe)

        # b h n c
        qkv = self.qkv(imgs).reshape(b , n , 3 , -1 ).permute(2,0,1,3)
        ...
        queries = self.queries_W(queries).reshape(b,queries_num , num_heads ,-1).permute(0,2,1,3)


        queriesAttn = self.softmax((queries * self.scale) @ k.transpose(-2,-1))
        # b h n c
        queries_v = queriesAttn @ v

        

        indicator  = self.ln(queries_v)

        q_attn = torch.softmax((indicator.transpose(-2,-1) / queries_num) @ indicator , dim = -2)
        # b h n c
        attn_out = q @ q_attn

        attn_out = attn_out.transpose(1,2).reshape(b , n ,c)
        queries_v = queries_v.transpose(1,2).reshape(b , queries_num, c)


        residual_img = residual_img + self.mlp(self.norm2(attn_out))
        residual_q = residual_q + self.mlp(self.norm2(queries_v))

        return residual_q , residual_img
...
@register_model
def tokengalerkin_v3_lessweight_t(args):
    model = VisSegNet(
