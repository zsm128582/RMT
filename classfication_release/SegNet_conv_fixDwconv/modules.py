...
class FeedForwardNetworkWithQuery(nn.Module):
    def __init__(
        self,
        embed_dim,
        ffn_dim,
    ...
    def forward(self, x: torch.Tensor , num_q , h ,w):
        '''
        x: (b Q+N c)
        '''
        b , L , _ = x.shape
        ...
        if self.dwconv is not None:
            queries = x[:,:num_q,:]
            img = x[:,num_q:,:].reshape(b,h,w,self.ffn_dim)
            img =  self.dwconv(img).reshape(b,-1,self.ffn_dim)
            x = torch.cat((queries , img) , dim=1)
