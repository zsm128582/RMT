...
from SimpleGalerkin import simple_attn
...
class BasicLayer(nn.Module):
    ...
    def __init__(
        ...
        # self.galerkin = simple_attn(embed_dim, num_heads)
        # self.addNorm = nn.LayerNorm(embed_dim)
    ...
    def forward(self, x):
        ...
        # # map我应该使用contiguous吗？
        # x = self.addNorm(x + self.galerkin(x.permute(0, 3, 1, 2)).permute(0, 2, 3, 1))
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, in_chans=3, embed_dim=96, norm_layer=None):
        ...
        # to 1/2 尺度 , 1/4 embed dim
        self.proj1 = nn.Sequential(
            nn.Conv2d(in_chans, embed_dim // 4, 3, 2, 1),
            nn.BatchNorm2d(embed_dim // 4),
            nn.GELU(),
        )
        # to 1/4 尺度
        self.proj2 = nn.Sequential(
            nn.Conv2d(embed_dim // 4, embed_dim // 4, 3, 2, 1),
            nn.BatchNorm2d(embed_dim//4),
            nn.GELU(),
        )
        # to 1/8 尺度
        ...
        self.galerkin = simple_attn(embed_dim , 1)
    ...
    def forward(self, x):
        ...
        x1 = F.interpolate(x1,size=(H//4,W//4),mode='area')
        x3 = F.interpolate(x3,size=(H//4,W//4),mode="bilinear",align_corners=False)
        x4 = F.interpolate(x4,size=(H//4,W//4),mode="bilinear",align_corners=False)

        x = torch.cat([x1,x2,x3,x4],dim=1)
        return self.galerkin(x).permute(0,2,3,1)  # (b h w c)
...
class VisRetNet(nn.Module):
    ...
    def __init__(
        ...
        self.head = (
            nn.Linear(projection, num_classes) if num_classes > 0 else nn.Identity()
        )
