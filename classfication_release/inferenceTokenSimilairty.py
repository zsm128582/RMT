...
def calRMTSimilarity(layer , block):
    def calHook(module , input , output):
        # print(input) # 输出为()
        # print(module) # 输出正确
        # print(output) # 存在输出
        # 均为bchw
        queries = input[0]  # ValueError: not enough values to unpack (expected 4, got 0)
        print(queries.shape)
        with torch.no_grad():
            token_embeddings = queries.squeeze(0) 
            token_embeddings = rearrange(token_embeddings , "  h w c-> (h w) c")
