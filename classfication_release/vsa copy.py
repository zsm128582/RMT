...
class VisionSparseAttention(nn.Module):
    ...
    def forward(self, x , rel_pos):
        ...
        # 计算精细q k attention
        # b h r w2 c
        attn = gatherAttention.apply(qpatch , kpatch , r_idx.int() , self.scale).softmax(dim = -1)
        attn = attn.contiguous()
        # print("atten shape:",attn.shape)
        # 得到最终输出
        # b heads region w2 c
        out_fine = AttentionWeighting.apply(attn , vpatch.float() , r_idx.int() , self.num_selected)
        out_fine = out_fine.flatten(2,3).contiguous()
