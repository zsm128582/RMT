        ...
        for j in range(tg.shape[-1]):
            for mat in mats:
                ...
                for k in now:
                    idx += 1
                    tmp += k*k
                    if tmp/total > threshold:#lower maybe
...
def main():
    ...
    if args.distributed:
        ...
        # args.rank, args.world_size = distributed.get_rank(), distributed.get_world_size()
    ...
    if args.pretrain_path is not None:
        ...
        # modellist = model.blocks
        # for bls in modellist :
        # #     res = inn.standard_continuous_dims(bls.inner_attn)
        # #     inn_model = inn.IntegralWrapper(init_from_discrete=True , 
        # #                                     permutation_config = {"class" :inn.permutation.NOptOutFiltersPermutation})(bls.inner_attn,(196,32,32),res,None)

        # #     inn_model.threshold = 0.95
        # #     inn_model.set_rate()
        # #     inn_model.set_distributions()
        # #     inn_model.set_size()
        # #     for g in inn_model.groups:
        # #         print("name:",g,"rate:",g.rate)
        #     for mat in bls.outer_attn.parameters():
        #         chf(mat)
        #     print("*************************************")
        # exit()
        # ###########end#############################
    ...
    if args.distributed:
        ...
        if has_apex and use_amp != 'native':
            ...
            model = NativeDDP(model, device_ids=[args.rank] ,find_unused_parameters = False)  # can use device str in Torch >= 1.1
    ...
    _logger.info("prepare train loader done")
    ...
    if args.rank == 0:
        ...
        output_dir = output_base + "/" + "train/" + exp_name
