...
__global__ void sparse_gemm_backward_q_kernel(
    const scalar_t* __restrict__ grad_out, // [B, heads, region, w2, topk] //FIXME :  [B, heads, region, topk , w2]
    ...
    int B, int heads, int region, int w2, int topk, int c) 
{
    ...
    if (index < total) {
        ...
        for (int j = 0; j < topk; j++) {
            ...
            grad_val += scale * grad_out_val * kpatch[kpatch_index];
        }
        
        grad_q[index] = grad_val;
    }
}
