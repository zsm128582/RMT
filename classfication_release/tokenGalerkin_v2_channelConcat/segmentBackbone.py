...
from .modules import PatchEmbed , PatchMerging , DWConv2d ,RetNetRelPos2d,MemoryEfficientSwish,theta_shift,RotateModule,simple_attn
...
class TwoWayAttentionBlock(nn.Module):
    def __init__(
        ...
        # self.mlp = MLP(
        #     embedding_dim*2, mlp_dim, embedding_dim, num_alyers=2, activation=activation
        # )

        self.mlp = FeedForwardNetwork(embedding_dim*2 , embedding_dim , mlp_dim, subconv=False )
    ...
    def forward(
        ...
        q = queries + query_pe
        k = imgs + img_pe
        attn_out , attn_weight = self.cross_attn_token_to_image(query= self.norm1(q), key=self.norm2(k), value=imgs,need_weights=True)
        ...
        q = queries + query_pe
        # k = keys + key_pe # 图像
        attn_out = self.cross_attn_image_to_token(x = imgs , token = q)

        temp = torch.concat((imgs , attn_out),dim=-1) 

        imgs =  imgs + self.mlp(self.norm3(temp))

        

        return queries, imgs,attn_weight
...
@register_model
def tokengalerkin_channelConcat_t_v2(args):
    model = VisSegNet(
